...
class TaskController:
    ...
    def submit_answers(self, profile: OAuthProfile, answers: List[Answer], space_id: Optional[str]) -> Tuple[str, Dict]:
        ...
        try:
            submission_response = self.task_gateway.submit_answers(username, answers_payload, space_id)
            return "Submission successful", submission_response
        except Exception as e:
            logger.exception(f"Error submitting answers via TaskGatewayPort for user {username}: {e}")
    ...
    def process_all_tasks(self, profile: OAuthProfile, space_id: Optional[str]) -> Tuple[str, Optional[pd.DataFrame]]:
        ...
        try:
            ...
            submission_status_msg, result_data = self.submit_answers(effective_profile, answers, space_id)
            ...
            logger.exception(f"Unexpected error in process_all_tasks for user {current_username}: {str(e)}")
